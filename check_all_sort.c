...
/*   Updated: 2024/11/17 20:53:12 by kaara            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_sort_a(struct stack *stack_a, struct stack *stack_b)
{
	...
	{
		if (!(stack_a->numbers[i] < stack_a->numbers[i + 1]) && (stack_b->numbers[1] != stack_b->max_or_min_value))
			return (false);
		i++;
	}
...
bool	check_sort_b(struct stack *stack_b)
{
	...
	{
		if (!(stack_b->numbers[i] > stack_b->numbers[i + 1]) && (stack_b->numbers[1] != stack_b->max_or_min_value))
			return (false);
		i++;
	}
...
// 	if (check_pivot(stack_a, stack_b))
// 		return (false);
// 	while (stack_a->flag-- >= 0)	//pivotができた時点でコピー
...
// 	if (stack_b->top <= 1)
// 		return (true);
// 	while (stack_b->flag-- >= 0)//pivotができた時点でコピー
// 		rev_rotate(stack_b);
